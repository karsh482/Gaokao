...
program_catalog_records.csv 用于承接招生专业目录 / 招生计划。它只表示计划信息，不是投档
或录取结果，不应写入 admission_records.csv。字段如下：

| 字段 | 说明 |
| --- | --- |
| record_number | 来源内记录序号 |
| exam_province | 考试/招生省份，例如贵州 |
| plan_year | 招生年份，例如 2026 |
| dataset_type | 数据类型，当前为 program_catalog |
| subject_category | 科类或选科口径，例如历史类、物理类 |
| admission_track | 招生大赛道，例如普通类、艺术类、体育类 |
| education_level | 层次，例如本科、高职（专科） |
| batch | 批次 |
| enrollment_type | 招生类型或项目 |
| school_code_in_exam_province | 考试省份招生目录中的院校代码 |
| school_name | 院校名称 |
| school_location | 来源中的院校所在地 |
| school_plan_count | 来源目录中的院校小计计划数 |
| major_code_in_exam_province | 考试省份招生目录中的专业代码 |
| major_name | 专业或方向名称 |
| selection_requirements | 再选科目要求 |
| enrollment_plan_count | 专业计划人数 |
| language | 外语语种要求 |
| duration | 学制 |
| tuition | 原始学费展示，可能为免费、待定或数值 |
| remarks | 来源备注 |
| source_file_id | 来源文件标识 |
| source_file_name | 来源文件名 |
| source_page | 来源页码 |
| source_column | 来源栏位 |
| source_line_start | 来源起始行 |
| source_line_end | 来源结束行 |
